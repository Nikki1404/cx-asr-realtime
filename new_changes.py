    ...
    *,
    ...
    try:
        ...
        if ref.strip():
            ...
            # Raw WER
            raw_wer = round(float(jiwer.wer(ref, hyp_safe)), 4)

            # Normalized WER
            norm_ref = pre_normalize(ref)
            norm_hyp = pre_normalize(hyp_safe)

            tok_ref = production_transform(norm_ref)
            tok_hyp = production_transform(norm_hyp)

            norm_wer = round(float(jiwer.wer(tok_ref, tok_hyp)), 4)
...
async def main():
    ...
    p.add_argument("--url", required=True)  # Always pass explicitly
...
python bench.py --url "wss://whisperstream.exlservice.com:3000/asr/realtime-custom-vad" --data-wav-root "C:\Users\re_nikitav\Documents\utils\utils\datasets\data\wav" --raw-librispeech-root "C:\Users\re_nikitav\Documents\utils\utils\datasets\data\raw\LibriSpeech" --max-files 5
